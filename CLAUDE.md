...
- 2026-08-04: Template pack Modules 1 + 2 drafted — `01-senior-year-timeline.md` (month-by-month money calendar) and `02-decoding-the-aid-letter.md` (four-bucket sort, worked two-offer comparison, Parent PLUS trap, comparison worksheet). Next up: Module 3, cost comparison framework.
...
- 2026-08-04: SOCIAL PIPELINE MAPPED + PARTIALLY VERIFIED. The "Tyson's Time" connector is Blotato (posting API). Live accounts: TikTok @tysons_time, IG @tysonstime, Threads @tysonstravels_rescuepitslife, YouTube (Tyson's Time Pit Bull Rescue), Facebook (no Page attached — unusable). Full runbook at `/shared/setup/social-posting-pipeline.md`. KEY CONSTRAINT: `database.blotato.io` is blocked by session egress policy, so a Claude session can neither upload media nor verify an upload — a 403 on the media URL means "can't see it," NOT "not there." Uploads must be run by Isaac from his Mac; the posting API itself works fine over MCP. Claude has NO Computer Use in this environment — no Mac access, no browser control, no filesystem beyond the container.
